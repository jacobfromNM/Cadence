...
- **School GPS coordinates:** Latitude and longitude of the school, entered by an administrator and used for parent proximity detection (geofencing)
...
- **Student codes:** Auto-generated 6-digit codes assigned to each student, used by parents to authenticate with the Parent Portal
...
- **Parent proximity events:** When a parent's device is detected within approximately 400 meters of the school, a timestamped proximity record is created and associated with the relevant student. **The parent's actual GPS coordinates are never transmitted to or stored on Cadence's servers.** Only the fact that the device crossed the proximity threshold is recorded.
...
- Parent GPS coordinates or precise location data
- Biometric data of any kind
- Payment information
- Device identifiers
- Any information about students beyond their name, classroom assignment, and auto-generated student code
...
- Student codes are used to authenticate parents with the Parent Portal, allowing them to view their child's pickup status without requiring a staff account
- School GPS coordinates are used to calculate whether a parent's device is within approximately 400 meters of the school, enabling automatic proximity alerts for staff and teachers
- Parent proximity events are used to display real-time alerts to staff and teachers so they can initiate pickup for an arriving parent
...
Parent proximity records persist in the database until dismissed by a staff or teacher through the in-app alert interface. Dismissed records are retained with a dismissal timestamp but are no longer surfaced to users. These records are not currently subject to automatic nightly deletion and may persist until the school's data is removed.

Student codes are assigned per student and persist as long as the student record exists. They can be regenerated by an administrator if needed.
...
- The Parent Portal uses browser-based geolocation. Location access is requested from the parent's device by the browser, not silently by the application. Parents can deny this permission; doing so disables proximity alerts but does not prevent them from viewing their child's pickup status.
- Student codes are short (6 digits) and anyone who obtains a valid school code and student code can access that student's pickup status. Codes should be treated as sensitive and shared only with the intended parent or guardian.
